...
dashapp = dash.Dash("intronret",url_base_pathname=f'{PAGE_PREFIX}/intronret/', meta_tags=META_TAGS, server=app, external_stylesheets=[dbc.themes.BOOTSTRAP, FONT_AWESOME], title=app.config["APP_TITLE"], assets_folder=app.config["APP_ASSETS"])# , assets_folder="/flaski/flaski/static/dash/")
...
def make_layout(pathname):
    eventlog = UserLogging(email=current_user.email, action="visit intronret")
    db.session.add(eventlog)
    db.session.commit()
    protected_content=html.Div(
        [
            make_navbar_logged("Intron Retention",current_user),
...
def make_app_content(session_id):
    header_access, msg_access = check_access( 'intronret' )
...
def read_file(contents,filename,last_modified):
    ...
    if "samples" not in exc.sheet_names :
        raise dash.exceptions.PreventUpdate
    if "intronRet" not in exc.sheet_names :
        raise dash.exceptions.PreventUpdate
    samples = pd.read_excel(io.BytesIO(decoded), sheet_name="samples")
    intronRet = pd.read_excel(io.BytesIO(decoded), sheet_name="intronRet")
    ...
    #     values_to_return.append(  intronRet[intronRet["Field"]==f]["Value"].tolist()[0]  )

    fields_on_file=intronRet["Field"].tolist()
    for f in fields_to_return:
...
def update_output(n_clicks, rows, email, group, folder, md5sums, project_title, organism, ercc, wget):
    header, msg = check_access( 'intronret' )
    ...
    if user_domain == "age.mpg.de" :
        send_submission_email(user=current_user, submission_type="intronRet", submission_file=os.path.basename(subdic["filename"]), attachment_path=subdic["filename"])
    else:
        send_submission_ftp_email(user=current_user, submission_type="intronRet", submission_file=os.path.basename(subdic["filename"]), attachment_path=subdic["filename"])
