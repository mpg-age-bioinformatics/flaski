...
dashapp = dash.Dash("sixteens",url_base_pathname=f'{PAGE_PREFIX}/sixteens/', meta_tags=META_TAGS, server=app, external_stylesheets=[dbc.themes.BOOTSTRAP, FONT_AWESOME], title="16S" , assets_folder=app.config["APP_ASSETS"])# , assets_folder="/flaski/flaski/static/dash/")
...
def generate_submission_file(rows, email,group,folder,md5sums,project_title,organism,ercc,fadapter,radapter,rarefy,lanes,wget, ftp):
    @cache.memoize(60*60*2) # 2 hours
    def _generate_submission_file(rows, email,group,folder,md5sums,project_title,organism,ercc,fadapter,radapter,rarefy,lanes, wget, ftp):
        df=pd.DataFrame()
        for row in rows:
            if row['Read 1'] != "" :
    ...
    return _generate_submission_file(rows, email,group,folder,md5sums,project_title,organism,ercc,fadapter,radapter,rarefy,lanes,wget, ftp)
...
def read_file(contents,filename,last_modified):
    ...
    values_to_return=[]
    fields_to_return=[ "email", "Group", "Folder", "md5sums", "Project title", "Organism", "ERCC","Forward adapter", "Reverse adapter", "Rarefy", "Lanes separately","wget", "ftp" ]
    # for f in fields_to_return:
    #     values_to_return.append(  SIXTEENS[SIXTEENS["Field"]==f]["Value"].tolist()[0]  )

    fields_on_file=SIXTEENS["Field"].tolist()
    for f in fields_to_return:
...
@dashapp.callback(
    ...
    State('ftp', 'value'),
    prevent_initial_call=True )
def update_output(n_clicks, rows, email, group, folder, md5sums, project_title, organism, ercc,fadapter,radapter,rarefy,lanes, wget, ftp):
    ...
    if not wget:
        wget="NONE"
    subdic=generate_submission_file(rows, email,group,folder,md5sums,project_title,organism,ercc,fadapter,radapter,rarefy,lanes, wget, ftp)
    ...
    if user_domain == "age.mpg.de" :
        send_submission_email(user=current_user, submission_type="16S", submission_tag=subdic["filename"], submission_file=None, attachment_path=None)
    else:
        ftp_user=send_submission_ftp_email(user=current_user, submission_type="16S", submission_tag=subdic["filename"], submission_file=None, attachment_path=subdic["filename"], ftp_user=ftp)
