...
def update_output(n_clicks, rows, email, group, folder, md5sums, project_title, organism, ercc, wget):
    ...
    if user_domain == "age.mpg.de" :
        ...
        send_submission_ftp_email(user=current_user, submission_type="alternativeSplicing", submission_file=os.path.basename(subdic["filename"]), attachment_path=subdic["filename"])
