...
dashapp = dash.Dash("asplicing",url_base_pathname=f'{PAGE_PREFIX}/asplicing/', meta_tags=META_TAGS, server=app, external_stylesheets=[dbc.themes.BOOTSTRAP, FONT_AWESOME], title="Alt. Splicing", assets_folder=app.config["APP_ASSETS"])# , assets_folder="/flaski/flaski/static/dash/")
...
def make_layout(pathname):
    eventlog = UserLogging(email=current_user.email, action="visit asplicing")
    db.session.add(eventlog)
    db.session.commit()
    protected_content=html.Div(
        [
            make_navbar_logged("Alternative Splicing",current_user),
...
def make_app_content(session_id):
    header_access, msg_access = check_access( 'asplicing' )
...
def read_file(contents,filename,last_modified):
    ...
    if "samples" not in exc.sheet_names :
        raise dash.exceptions.PreventUpdate
    if "alternativeSplicing" not in exc.sheet_names :
        raise dash.exceptions.PreventUpdate
    samples = pd.read_excel(io.BytesIO(decoded), sheet_name="samples")
    alternativeSplicing = pd.read_excel(io.BytesIO(decoded), sheet_name="alternativeSplicing")
...
def update_output(n_clicks, rows, email, group, folder, md5sums, project_title, organism, ercc, wget, ftp):
    header, msg = check_access( 'asplicing' )
    ...
    # if user_domain == "age.mpg.de" :
    #     send_submission_email(user=current_user, submission_type="alternativeSplicing", submission_tag=subdic["filename"],  submission_file=None, attachment_path=None)
    # else:
    ftp_user=send_submission_ftp_email(user=current_user, submission_type="alternativeSplicing", submission_tag=subdic["filename"], submission_file=None, attachment_path=subdic["filename"], ftp_user=ftp)
