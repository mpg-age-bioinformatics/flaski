...
from pyflaski.pca import make_figure, figure_defaults
...
dashapp = dash.Dash("pca",url_base_pathname=f'{PAGE_PREFIX}/pca/', meta_tags=META_TAGS, server=app, external_stylesheets=[dbc.themes.BOOTSTRAP, FONT_AWESOME], title=app.config["APP_TITLE"], assets_folder=app.config["APP_ASSETS"])# , assets_folder="/flaski/flaski/static/dash/")
...
def run_pca_and_cache(df, pa, cache):
    @cache.memoize(timeout=3600)
    def _run_pca_and_cache(df, pa, cache):
        projected , features = make_figure(df,pa)
        pca_results={ "projected": projected.to_json(), "features": features.to_json() }
        return pca_results
    return _run_pca_and_cache(df, pa, cache)
...
def make_layout(pathname):
    eventlog = UserLogging(email=current_user.email, action="visit pca")
    db.session.add(eventlog)
    db.session.commit()
    protected_content=html.Div(
        [
            make_navbar_logged("PCA",current_user),
            ...
            [   
                ...
                dbc.Card(
                    [
                        dbc.CardHeader(
                            html.H2(
                                dbc.Button( "Reduction", color="black", id={'type':"dynamic-card","index":"reduction"}, n_clicks=0,style={ "margin-bottom":"5px","width":"100%"}),
                            ),
                        ...
                        dbc.Collapse(
                            ...
                            id={'type':"collapse-dynamic-card","index":"reduction"},
                            is_open=False,
                        ),
                    ],
                ...
                [
                    ...
                    dbc.Modal(
                        [
                            ...
                            dbc.ModalBody(
                                [
                                    dcc.Input(id='excel-filename', value="pca.xlsx", type='text', style={"width":"100%"})
                                ]
                            ),
                            dbc.ModalFooter(
                                dbc.Button(
                    ...
                    dbc.Modal(
                        [
                            ...
                            dbc.ModalBody(
                                [
                                    dcc.Input(id='tsv-filename', value="pca.tsv", type='text', style={"width":"100%"})
                                ]
                            ),
                            dbc.ModalFooter(
                                dbc.Button(
                    ...
                    dbc.Modal(
                        [
                            ...
                            dbc.ModalBody(
                                [
                                    dcc.Input(id='export-filename', value="pca.json", type='text', style={"width":"100%"})
                                ]
                            ),
                            dbc.ModalFooter(
                                dbc.Button(
...
def read_input_file(contents,filename,last_modified,session_id):
    ...
    try:
        if filename.split(".")[-1] == "json":
            app_data=parse_import_json(contents,filename,last_modified,current_user.id,cache, "pca")
            ...
            df=parse_table(contents,filename,last_modified,current_user.id,cache,"pca")
        ...
        toast=make_except_toast("There was a problem reading your input file:","read_input_file", e, current_user,"pca")
...
def make_fig_output(n_clicks,export_click,save_session_btn,saveas_session_btn,save_excel_btn, save_tsv_btn, scatterplot_session_btn, session_id,contents,filename,last_modified,export_filename,excel_filename,tsv_filename,upload_data_text, *args):
    ...
    try:
        ...
        tb_str=''.join(traceback.format_exception(None, e, e.__traceback__))
        toast=make_except_toast("There was a problem parsing your input.","make_fig_output", e, current_user,"pca")
    ...
    if button_id == "save-session-btn" :
        try:
            ...
            tb_str=''.join(traceback.format_exception(None, e, e.__traceback__))
            toast=make_except_toast("There was a problem saving your file.","save", e, current_user,"pca")
    ...
    if button_id == "excel-filename-download":
        if not excel_filename:
            excel_filename=secure_filename("pca_results.%s.xlsx" %(time.strftime("%Y%m%d_%H%M%S", time.localtime())))
        excel_filename=secure_filename(excel_filename)
        if excel_filename.split(".")[-1] != "xlsx":
            excel_filename=f'{excel_filename}.xlsx'  

        pca_results=run_pca_and_cache(df, pa, cache)
        projected = pd.read_json(pca_results["projected"]) 
        features = pd.read_json(pca_results["features"]) 
        

        import io
        output = io.BytesIO()
        writer= pd.ExcelWriter(output)
        projected.to_excel(writer, sheet_name = 'pca results', index = False)
    ...
    if button_id == "tsv-filename-download":
        if not tsv_filename:
            tsv_filename=secure_filename("pca_results.%s.tsv" %(time.strftime("%Y%m%d_%H%M%S", time.localtime())))
    ...
    if button_id == "scatterplot-session-btn":
        ...
        session_data={ "session_data": {"app": { "scatterplot": {"filename":"<from PCA app>.json" ,'last_modified':last_modified,"df":pca_results["projected"],"pa":sp_pa} } } }
        session_data["APP_VERSION"]=app.config['APP_VERSION']
    ...
    try:
        ...
        #     df[col]=df[col].apply(lambda x: str(x)[:40]+"..")
        projected_ = make_table(projected, "pca_results", fixed_columns = False)
        features_ = make_table(features, "feature_table", fixed_columns = False)
        
        fig = dcc.Tabs([
            dcc.Tab([projected_], label = "Projection", id = "tab-projection"),
            dcc.Tab([features_], label = "Features", id = "tab-features"),
        ])
        ...
        tb_str=''.join(traceback.format_exception(None, e, e.__traceback__))
        toast=make_except_toast("There was a problem generating your output.","make_fig_output", e, current_user,"pca")
...
def help_email(n,tb_str, session_data):
    ...
    if n : 
        ...
        ask_for_help(tb_str,current_user, "pca", session_data)
