...
def generate_submission_file(rows, email,group,folder,md5sums,project_title,organism,ercc, model,targeted,wget, ftp):
    ...
    def _generate_submission_file(rows, email,group,folder,md5sums,project_title,organism,ercc,model,targeted,wget,ftp):
        ...
        df_=pd.DataFrame({"Field":["email","Group","Folder","md5sums","Project title", "Organism", "ERCC", "model","targeted Exomes","wget","ftp"],\
                          "Value":[email,group,folder,md5sums,project_title, organism, ercc,model,targeted, wget, ftp]}, index=list(range(11)))
        df=df.to_json()
        df_=df_.to_json()
