...
dashapp = dash.Dash("mirna",url_base_pathname=f'{PAGE_PREFIX}/mirna/', meta_tags=META_TAGS, server=app, external_stylesheets=[dbc.themes.BOOTSTRAP, FONT_AWESOME], title=app.config["APP_TITLE"], assets_folder=app.config["APP_ASSETS"])# , assets_folder="/flaski/flaski/static/dash/")
...
def make_layout(pathname):
    eventlog = UserLogging(email=current_user.email, action="visit mirna")
...
def make_app_content(session_id):
    header_access, msg_access = check_access( 'mirna' )
...
def read_file(contents,filename,last_modified):
    ...
    if "samples" not in exc.sheet_names :
        raise dash.exceptions.PreventUpdate
    if "miRNAseq" not in exc.sheet_names :
        raise dash.exceptions.PreventUpdate
    samples = pd.read_excel(io.BytesIO(decoded), sheet_name="samples")
    miRNAseq = pd.read_excel(io.BytesIO(decoded), sheet_name="miRNAseq")
    ...
    fields_on_file=miRNAseq["Field"].tolist()
    for f in fields_to_return:
...
def update_output(n_clicks, rows, email, group, folder, md5sums, project_title, organism, ercc,adapter, wget):
    ...
    metadata.to_excel(EXCout,"miRNAseq",index=None)
    EXCout.save()

    if user_domain == "age.mpg.de" :
        send_submission_email(user=current_user, submission_type="miRNAseq", submission_file=os.path.basename(subdic["filename"]), attachment_path=subdic["filename"])
    else:
        send_submission_ftp_email(user=current_user, submission_type="miRNAseq", submission_file=os.path.basename(subdic["filename"]), attachment_path=subdic["filename"])
