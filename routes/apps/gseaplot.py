...
dashapp = dash.Dash("gseaplot",url_base_pathname=f'{PAGE_PREFIX}/gseaplot/', meta_tags=META_TAGS, server=app, external_stylesheets=[dbc.themes.BOOTSTRAP, FONT_AWESOME], title="GSEA Plot", assets_folder=app.config["APP_ASSETS"])# , assets_folder="/flaski/flaski/static/dash/")
...
def download_pdf(n_clicks,graph, pdf_filename):
    ...
    eventlog = UserLogging(email=current_user.email,action="download figure gseaplot")
    db.session.add(eventlog)
    db.session.commit()
