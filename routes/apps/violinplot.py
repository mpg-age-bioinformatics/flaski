...
dashapp = dash.Dash("violinplot",url_base_pathname=f'{PAGE_PREFIX}/violinplot/', meta_tags=META_TAGS, server=app, external_stylesheets=[dbc.themes.BOOTSTRAP, FONT_AWESOME], title="Violin plot", assets_folder=app.config["APP_ASSETS"])# , assets_folder="/flaski/flaski/static/dash/")
...
# List components moved up to ensure all the components exist, which is needed to trigger callback
read_input_updates=[
    'hue',
    'vp_label',
    'fig_width',
...
placeholders_read_input_updates = html.Div([
    dcc.Input(id=id_, value='', style={'display': 'none'})
    for id_ in read_input_updates
])
...
dashapp.layout=html.Div( 
    [ 
        ...
        placeholders_read_input_updates
    ] 
)
...
# Dynamically assign outputs from `read_input_updates`
