...
dashapp = dash.Dash("chipseq",url_base_pathname=f'{PAGE_PREFIX}/chipseq/', meta_tags=META_TAGS, server=app, external_stylesheets=[dbc.themes.BOOTSTRAP, FONT_AWESOME], title="ChIP seq" , assets_folder=app.config["APP_ASSETS"])# , assets_folder="/flaski/flaski/static/dash/")
...
def update_output(n_clicks,rows_atac,rows_input,email,group,folder,md5sums,project_title,organism,ercc,seq, adapter,macs2,mito,wget, ftp ):
    ...
    # else:
    ftp_user=send_submission_ftp_email(user=current_user, submission_type="ChIPseq", submission_tag=subdic["filename"], submission_file=None, attachment_path=subdic["filename"], ftp_user=ftp)
