...
dashapp = dash.Dash("irfinder",url_base_pathname=f'{PAGE_PREFIX}/irfinder/', meta_tags=META_TAGS, server=app, external_stylesheets=[dbc.themes.BOOTSTRAP, FONT_AWESOME], title=app.config["APP_TITLE"], assets_folder=app.config["APP_ASSETS"])# , assets_folder="/flaski/flaski/static/dash/")
...
def update_output(n_clicks, rows, email, group, folder, md5sums, project_title, organism, ercc, wget):
    ...
    if user_domain == "age.mpg.de" :
        ...
        send_submission_ftp_email(user=current_user, submission_type="IRfinder", submission_tag=subdic["filename"], submission_file=None, attachment_path=None)
