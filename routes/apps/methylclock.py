...
dashapp = dash.Dash("methylclock",url_base_pathname=f'{PAGE_PREFIX}/methylclock/', meta_tags=META_TAGS, server=app, external_stylesheets=[dbc.themes.BOOTSTRAP, FONT_AWESOME], title=app.config["APP_TITLE"], assets_folder=app.config["APP_ASSETS"])# , assets_folder="/flaski/flaski/static/dash/")
...
def make_app_content(session_id):
    header_access, msg_access = check_access( 'methylclock' )
...
def read_file(contents,filename,last_modified):
    ...
    if "samples" not in exc.sheet_names :
        raise dash.exceptions.PreventUpdate
    if "methylclock" not in exc.sheet_names :
        raise dash.exceptions.PreventUpdate
    samples = pd.read_excel(io.BytesIO(decoded), sheet_name="samples")
    methylclock = pd.read_excel(io.BytesIO(decoded), sheet_name="methylclock")
    ...
    fields_on_file=methylclock["Field"].tolist()
    for f in fields_to_return:
...
def update_output(n_clicks, rows, email, group, folder, md5sums, project_title, organism,rrbs, wget):
    ...
    if user_domain == "age.mpg.de" :
        send_submission_email(user=current_user, submission_type="methylclock", submission_file=os.path.basename(subdic["filename"]), attachment_path=subdic["filename"])
    else:
        send_submission_ftp_email(user=current_user, submission_type="methylclock", submission_file=os.path.basename(subdic["filename"]), attachment_path=subdic["filename"])
