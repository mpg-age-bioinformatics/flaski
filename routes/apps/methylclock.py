...
def update_output(n_clicks, rows, email, group, folder, md5sums, project_title, organism,rrbs, wget):
    ...
    if user_domain == "age.mpg.de" :
        send_submission_email(user=current_user, submission_type="methylclock", submission_file=None, attachment_path=None)
    else:
        send_submission_ftp_email(user=current_user, submission_type="methylclock", submission_file=None, attachment_path=None)
