...
def generate_submission_file(rows_atac, email,group,folder,md5sums,project_title,organism,seq, adapter, mito, genes, wget, ftp):
    @cache.memoize(60*60*2) # 2 hours
    def _generate_submission_file(rows_atac, email,group,folder,md5sums,project_title,organism,seq, adapter, mito, genes, wget, ftp):
        df=pd.DataFrame()
    ...
    return _generate_submission_file(rows_atac, email,group,folder,md5sums,project_title,organism,seq,adapter,mito,genes,wget,ftp)
...
def make_app_content(session_id):
    ...
    input_df=pd.DataFrame(columns=["ATACseq Sample"])
    input_eg_df=pd.DataFrame( { "ATACseq Sample" :["WT_control_1","WT_control_2","MUT_control_1","MUT_control_2","WT_treated_1","WT_treated_2","MUT_treated_1","MUT_treated_2"] } )


    # generate dropdown options
    organisms=["celegans","mmusculus","hsapiens","dmelanogaster", "nfurzeri", "scerevisae"] # "c_albicans_sc5314"
    organisms_=make_options(organisms)
    yes_no_=make_options(["yes","no"])
    seq_=make_options(["single","paired"])
    atacseq_options=make_options(["YES","NO"])
...
def update_output(n_clicks,rows_atac,email,group,folder,md5sums,project_title,organism,seq, adapter, mito, genes, wget, ftp ):
