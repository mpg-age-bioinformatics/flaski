...
from pyflaski.tsne import make_figure, figure_defaults
...
dashapp = dash.Dash("tsne",url_base_pathname=f'{PAGE_PREFIX}/tsne/', meta_tags=META_TAGS, server=app, external_stylesheets=[dbc.themes.BOOTSTRAP, FONT_AWESOME], title=app.config["APP_TITLE"], assets_folder=app.config["APP_ASSETS"])# , assets_folder="/flaski/flaski/static/dash/")
...
def run_tsne_and_cache(df, pa, cache):
    @cache.memoize(timeout=3600)
    def _run_tsne_and_cache(df, pa, cache):
        projected = make_figure(df,pa)
        tsne_results={ "projected": projected.to_json() }
        return tsne_results
    return _run_tsne_and_cache(df, pa, cache)
...
def make_layout(pathname):
    eventlog = UserLogging(email=current_user.email, action="visit tsne")
    db.session.add(eventlog)
    db.session.commit()
    protected_content=html.Div(
        [
            make_navbar_logged("tSNE",current_user),
                        ...
                        dbc.Collapse(
                            dbc.CardBody(
                                ...
                                dbc.Row(
                                    [
                                    dbc.Col(
                                        dbc.Label("Perplexity:",html_for="perplexity", style={"margin-top":"5px"}),
                                        style={"textAlign":"left","padding-right":"2px"},
                                        width=8
                                    ),
                                    dbc.Col(
                                        dcc.Input(value=pa["perplexity"], id='perplexity',style=card_input_style),
                                        width=4
                                    ),
                                    ],
                                    className="g-1",
                ...
                [
                    ...
                    dbc.Modal(
                        [
                            ...
                            dbc.ModalBody(
                                [
                                    dcc.Input(id='excel-filename', value="tsne.xlsx", type='text', style={"width":"100%"})
                                ]
                            ),
                            dbc.ModalFooter(
                                dbc.Button(
                    ...
                    dbc.Modal(
                        [
                            ...
                            dbc.ModalBody(
                                [
                                    dcc.Input(id='tsv-filename', value="tsne.tsv", type='text', style={"width":"100%"})
                                ]
                            ),
                            dbc.ModalFooter(
                                dbc.Button(
                    ...
                    dbc.Modal(
                        [
                            ...
                            dbc.ModalBody(
                                [
                                    dcc.Input(id='export-filename', value="tsne.json", type='text', style={"width":"100%"})
                                ]
                            ),
                            dbc.ModalFooter(
                                dbc.Button(
...
def read_input_file(contents,filename,last_modified,session_id):
    ...
    try:
        if filename.split(".")[-1] == "json":
            app_data=parse_import_json(contents,filename,last_modified,current_user.id,cache, "tsne")
            ...
            df=parse_table(contents,filename,last_modified,current_user.id,cache,"tsne")
        ...
        toast=make_except_toast("There was a problem reading your input file:","read_input_file", e, current_user,"tsne")
...
def make_fig_output(n_clicks,export_click,save_session_btn,saveas_session_btn,save_excel_btn, save_tsv_btn, scatterplot_session_btn, session_id,contents,filename,last_modified,export_filename,excel_filename,tsv_filename,upload_data_text, *args):
    ...
    try:
        ...
        session_data={ "session_data": {"app": { "tsne": {'last_modified':last_modified,"pa":pa} } } }
        session_data["APP_VERSION"]=app.config['APP_VERSION']
        
    except Exception as e:
        tb_str=''.join(traceback.format_exception(None, e, e.__traceback__))
        toast=make_except_toast("There was a problem parsing your input.","make_fig_output", e, current_user,"tsne")
    ...
    if button_id == "save-session-btn" :
        try:
            ...
            tb_str=''.join(traceback.format_exception(None, e, e.__traceback__))
            toast=make_except_toast("There was a problem saving your file.","save", e, current_user,"tsne")
    ...
    if button_id == "excel-filename-download":
        if not excel_filename:
            excel_filename=secure_filename("tsne_results.%s.xlsx" %(time.strftime("%Y%m%d_%H%M%S", time.localtime())))
        excel_filename=secure_filename(excel_filename)
        if excel_filename.split(".")[-1] != "xlsx":
            excel_filename=f'{excel_filename}.xlsx'  

        tsne_results=run_tsne_and_cache(df, pa, cache)
        projected = pd.read_json(tsne_results["projected"]) 

        import io
        output = io.BytesIO()
        writer= pd.ExcelWriter(output)
        projected.to_excel(writer, sheet_name = 'tsne results', index = False)
        writer.save()
    ...
    if button_id == "tsv-filename-download":
        if not tsv_filename:
            tsv_filename=secure_filename("tsne_results.%s.tsv" %(time.strftime("%Y%m%d_%H%M%S", time.localtime())))
    ...
    if button_id == "scatterplot-session-btn":
        ...
        session_data={ "session_data": {"app": { "scatterplot": {"filename":"<from tSNE app>.json" ,'last_modified':last_modified,"df":tsne_results["projected"],"pa":sp_pa} } } }
        session_data["APP_VERSION"]=app.config['APP_VERSION']
    ...
    try:
        ...
        fig=make_table(projected, "tsne_results", fixed_columns = False)
        ...
        tb_str=''.join(traceback.format_exception(None, e, e.__traceback__))
        toast=make_except_toast("There was a problem generating your output.","make_fig_output", e, current_user,"tsne")
...
def help_email(n,tb_str, session_data):
    ...
    if n : 
        ...
        ask_for_help(tb_str,current_user, "tsne", session_data)
