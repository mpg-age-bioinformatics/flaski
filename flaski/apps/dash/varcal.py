...
CURRENTAPP="varcal"
navbar_title="Variant calling submission form"
...
def generate_submission_file(rows, email,group,folder,md5sums,project_title,organism,ercc,model,  targeted):
    @cache.memoize(60*60*2) # 2 hours
    def _generate_submission_file(rows, email,group,folder,md5sums,project_title,organism,ercc,model,  targeted):
        df=pd.DataFrame()
        for row in rows:
            if row['Read 1'] != "" :
        ...
        df_=pd.DataFrame({"Field":["email","Group","Folder","md5sums","Project title", "Organism", "ERCC", "model","targeted Exomes"],\
                          "Value":[email,group,folder,md5sums,project_title, organism, ercc, model,  targeted]}, index=list(range(9)))
        df=df.to_json()
        df_=df_.to_json()
        filename=make_submission_file(".variantCalling.xlsx")

        return {"filename": filename, "samples":df, "metadata":df_}
    return _generate_submission_file(rows, email,group,folder,md5sums,project_title,organism,ercc,model,  targeted)
...
example_input=pd.DataFrame( { "Sample":["AWT","cell line A treated 1","cell line A treated 2","cell line A treated 3",
                            ...
                            "Read 2": [ "A003450092_131904_S2_L002_R2_001.fastq.gz",
                                        "A006850092_131904_S21_L002_R2_001.fastq.gz",
...
def update_output(session_id, n_clicks, rows, email,group,folder,md5sums,project_title,organism,ercc, model, targeted):
...
#### !! You have no access to this App !!
    ...
    subdic=generate_submission_file(rows, email,group,folder,md5sums,project_title,organism,ercc, model, targeted)
