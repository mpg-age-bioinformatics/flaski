...
from flaski.apps.main.mds import make_figure, figure_defaults
...
@app.route('/mds/<download>', methods=['GET', 'POST'])
    ...
    if request.method == 'POST':
        ...
        try:
            if request.files["inputsessionfile"] :
                msg, plot_arguments, error=read_session_file(request.files["inputsessionfile"],"mds")
                if error:
                    flash(msg,'error')
                    return render_template('/apps/mds.html' , filename=session["filename"],apps=apps, **plot_arguments)
                flash(msg,"info")

            if request.files["inputargumentsfile"] :
                msg, plot_arguments, error=read_argument_file(request.files["inputargumentsfile"],"mds")
                if error:
                    flash(msg,'error')
                    return render_template('/apps/mds.html' , filename=session["filename"], apps=apps, **plot_arguments)
                flash(msg,"info")
            ...
            if inputfile:
                ...
                if allowed_file(inputfile.filename):
                    ...
                    return render_template('/apps/mds.html' , filename="Select file..", apps=apps, **plot_arguments)
            ...
            # try:
            df_mds=make_figure(df,plot_arguments)

            df_mds=df_mds.astype(str)
            session["df_mds"]=df_mds.to_json()

            #features=features.astype(str)
            #session["features"]=features.to_json()

            df_selected=df_mds[:50]
            ...
            return render_template('/apps/mds.html', table_headers=table_headers, table_contents=df_selected, filename=filename, apps=apps, **plot_arguments)
        ...
        if download == "download":
            ...
            # READ INPUT DATA FROM SESSION JSON
            df_mds=pd.read_json(session["df_mds"])
            #features=pd.read_json(session["features"])

            # CALL FIGURE FUNCTION

            eventlog = UserLogging(email=current_user.email,action="download table mds values")
            ...
            if plot_arguments["downloadf"] == "xlsx":
                ...
                df_mds.to_excel(EXC,sheet_name="mds", index=None)
                ...
                return Response(df_mds.to_csv(sep="\t"), mimetype="text/csv", headers={"Content-disposition": "attachment; filename=%s.tsv" %plot_arguments["downloadn"]})
        ...
        if download == "scatter":
            ...
            # INITIATE SESSION
            session["filename"]="<from MDS>"
            plot_arguments=iscatterplot.figure_defaults()
            session["COMMIT"]=app.config['COMMIT']
            session["app"]="iscatterplot"

            df_mds=df_mds.astype(str)
            session["df"]=df_mds.to_json()

            plot_arguments["xcols"]=df_mds.columns.tolist()
            plot_arguments["ycols"]=df_mds.columns.tolist()
            plot_arguments["groups"]=plot_arguments["groups"]+df_mds.columns.tolist()
            plot_arguments["labels_col"]=df_mds.columns.tolist()

            plot_arguments["xvals"]=df_mds.columns.tolist()[1]
            plot_arguments["yvals"]=df_mds.columns.tolist()[2]
            plot_arguments["labels_col_value"]=df_mds.columns.tolist()[0]
            plot_arguments["title"]="MDS"
            plot_arguments["xlabel"]=df_mds.columns.tolist()[1]
            plot_arguments["ylabel"]=df_mds.columns.tolist()[2]

            session["plot_arguments"]=plot_arguments

            return render_template('/apps/iscatterplot.html', filename=session["filename"], apps=apps, **plot_arguments)

        return render_template('apps/mds.html',  filename=session["filename"], apps=apps, **session["plot_arguments"])            