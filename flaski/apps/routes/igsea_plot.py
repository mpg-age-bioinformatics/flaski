...
from flaski.apps.main.igsea_plot import make_figure, figure_defaults
...
@app.route('/igsea_plot/<download>', methods=['GET', 'POST'])
@app.route('/igsea_plot', methods=['GET', 'POST'])
@login_required
def igsea_plot(download=None):
    """ 
    ...
    reset_info=check_session_app(session,"igsea_plot",apps)
    if reset_info:
    ...
    if request.method == 'POST' :
        ...
        try:
            ...
            if request.files["inputsessionfile"] :
                msg, plot_arguments, error=read_session_file(request.files["inputsessionfile"],"igsea_plot")
                if error:
                    flash(msg,'error')
                    return render_template('/apps/igsea_plot.html' , filename=session["filename"],apps=apps, **plot_arguments)
                flash(msg,"info")

            if request.files["inputargumentsfile"] :
                msg, plot_arguments, error=read_argument_file(request.files["inputargumentsfile"],"igsea_plot")
                if error:
                    flash(msg,'error')
                    return render_template('/apps/igsea_plot.html' , filename=session["filename"], apps=apps, **plot_arguments)
                flash(msg,"info")
            ...
            if inputfile:
                ...
                if allowed_file(inputfile.filename):
                    ...
                    if (session["plot_arguments"]["xvals"] not in cols) | (session["plot_arguments"]["yvals"] not in cols):
                        ...
                        flash(sometext,'info')
                        return render_template('/apps/igsea_plot.html' , filename=filename, apps=apps,**plot_arguments)
                    ...
                    flash(error_msg,'error')
                    return render_template('/apps/igsea_plot.html' , filename="Select file..", apps=apps, **plot_arguments)
            ...
            return render_template('/apps/igsea_plot.html', figure_url=figure_url, filename=filename, apps=apps, **plot_arguments)

        except Exception as e:
            tb_str=handle_exception(e,user=current_user,eapp="igsea_plot",session=session)
        ...
        if download == "download":
            ...
            eventlog = UserLogging(email=current_user.email,action="download figure igsea_plot")
        ...
        return render_template('apps/igsea_plot.html',  filename=session["filename"], apps=apps, **session["plot_arguments"])